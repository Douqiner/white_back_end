...
def confirm_arrival():
    ...
    # 检查是否所有参与者都已到达
    all_arrived = True
    if order.user1 and not (order_status.user1_arrived or (is_driver and order_status.driver_arrived)):
        all_arrived = False
    if order.user2 and not order_status.user2_arrived:
        all_arrived = False
    if order.user3 and not order_status.user3_arrived:
        all_arrived = False
    if order.user4 and not order_status.user4_arrived:
        all_arrived = False
    if not order_status.driver_arrived:
        all_arrived = False

    # 如果所有人都到达，更新订单状态为进行中
    if all_arrived and order_status.status == 0:
...
def confirm_destination():
    ...
    if is_driver:
        ...
        if order_status.status == 0:
            # 如果当前是"未开始"状态，设置为"进行中"
            order_status.status = 1  # 进行中
            message = "行程开始成功"
        elif order_status.status == 1:
            # 如果当前是"进行中"状态，设置为"已完成"
